...
public interface IMcpClient : IDisposable
{
    ...
    Task<JsonElement> ListResourcesAsync(CancellationToken ct);
    Task<JsonElement> ReadResourceAsync(string uri, CancellationToken ct);
}
    ...
    CancellationToken ct)
{
    foreach (var config in servers.Where(s => s.Enabled))
    {
        try
        {
            IMcpClient client = !string.IsNullOrEmpty(config.Url)
                ? await McpSseClient.ConnectAsync(config, ct)
                : await McpClient.ConnectAsync(config, ct);
            ...
            var label = !string.IsNullOrEmpty(config.Url) ? " (SSE)" : "";
            _warn?.Invoke($"MCP: Connected to {config.Name}{label} — {toolsResult.GetProperty("tools").GetArrayLength()} tools");
        }
        catch (Exception ex)
        {
