...
#   AMD_IGPU_MODE=1       Force AMD iGPU mode (writes Vulkan docker-compose override)
...
CTX_CPU=262144       # CPU/Vulkan q8_0 — 30% of 128GB+ RAM (~38GB for KV cache)
CTX_VISION=204800    # +mmproj — reduced from CTX_CPU to fit vision encoder in RAM
...
# Any amdgpu.gttsize value counts — the GTT window is RAM-dependent (opt script)
# and may also have been tuned manually.
if [[ "${AMD_IGPU_MODE:-0}" = "1" && "${OPENMONO_ROLE:-}" != "agent" ]]; then
...
if [ "$OPENMONO_ROLE" != "agent" ]; then
    if [ "${GPU_MODE:-0}" = "1" ]; then
        ...
        if [ -r /proc/meminfo ]; then
            # /proc/meminfo instead of `free` — free's output labels are localized
...
EOF
    ...
    # Thresholds are below the nominal size (120 for 128GB, 60 for 64GB) since
    # the kernel reports usable RAM minus firmware/iGPU carve-outs.
    _TOTAL_RAM_GB=$(awk '/^MemTotal/{print int($2/1048576)}' /proc/meminfo)
    _TOTAL_RAM_GB=${_TOTAL_RAM_GB:-0}
    if [ "$_TOTAL_RAM_GB" -ge 120 ]; then
        _SHM_SIZE="32gb"
        _CTX_MAX=262144
    elif [ "$_TOTAL_RAM_GB" -ge 60 ]; then
        _SHM_SIZE="16gb"
        _CTX_MAX=196608
    else
        _SHM_SIZE="8gb"
    ...
    # so resolve them from the host instead of hardcoding.
    _VIDEO_GID=$(getent group video 2>/dev/null | cut -d: -f3)
    _RENDER_GID=$(getent group render 2>/dev/null | cut -d: -f3)
    # Fallback: owning group of the render node device
    [ -z "$_RENDER_GID" ] && _RENDER_GID=$(stat -c '%g' /dev/dri/renderD128 2>/dev/null || true)
    _VIDEO_GID="${_VIDEO_GID:-44}"
    _RENDER_GID="${_RENDER_GID:-109}"
    detail "Host GIDs: video=$_VIDEO_GID render=$_RENDER_GID"

    # Note: models volume and port mapping come from the base docker-compose.yml
    # (../models:/models:ro) — do not redefine them here.
    cat > "$OVERRIDE_FILE" <<EOF
# AMD Radeon iGPU / Vulkan configuration (auto-generated by install.sh)
# Detected: ${_TOTAL_RAM_GB}GB RAM → shm_size=${_SHM_SIZE}, max_ctx=${_CTX_MAX}
services:
  llama-server:
